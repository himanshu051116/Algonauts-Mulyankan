...
Write-Host "Mulyankan 0.6.3 evidence-gated scoring upgrade" -ForegroundColor Green
Write-Host "Source: $RepoRoot"

if (-not (Test-Path -LiteralPath (Join-Path $RepoRoot ".env"))) {
    throw ".env is missing. Copy the working 0.6.1 .env into this new 0.6.3 source folder before upgrading."
}
...
$backupDir = Join-Path $RepoRoot "backups\upgrade-0.6.3-$timestamp"
New-Item -ItemType Directory -Path $backupDir -Force | Out-Null
$dumpPath = Join-Path $backupDir "postgres-pre-0.6.3.dump"

Invoke-Checked "Create PostgreSQL custom-format backup" {
    docker compose exec -T postgres sh -lc 'rm -f /tmp/postgres-pre-0.6.3.dump && pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Fc -f /tmp/postgres-pre-0.6.3.dump'
}
Invoke-Checked "Copy PostgreSQL backup to host" {
    docker compose cp "postgres:/tmp/postgres-pre-0.6.3.dump" $dumpPath
}
...
Copy-Item -LiteralPath (Join-Path $RepoRoot ".env") -Destination (Join-Path $backupDir ".env.pre-0.6.3") -Force
Copy-Item -LiteralPath (Join-Path $RepoRoot "docker-compose.yml") -Destination (Join-Path $backupDir "docker-compose.pre-0.6.3.yml") -Force
...
if ($health.version -ne "0.6.3") {
    throw "Backend reported version '$($health.version)' instead of 0.6.3."
}
...
if ($values["migration_head"] -ne "20260708_scoring_safety") {
...
Write-Host "Do not delete the 0.6.1 source or this backup until end-to-end evaluation tests pass."
