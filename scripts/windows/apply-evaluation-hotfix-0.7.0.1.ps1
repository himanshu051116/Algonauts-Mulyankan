...
Write-Host "Mulyankan 0.7.0.1 evaluation-persistence hotfix" -ForegroundColor Green
if (-not (Test-Path (Join-Path $RepoRoot ".env"))) {
    throw ".env is missing. Copy the working 0.7.0 .env into this hotfix source folder."
}
...
for ($attempt = 1; $attempt -le 30; $attempt++) {
    try {
        ...
        if ($health.version -eq "0.7.0.1" -and $readiness.status -eq "ready") {
            $ready = $true
            break
        }
    } catch {
...
if (-not $ready) {
    docker compose ps -a
    throw "The hotfix services did not become ready with version 0.7.0.1."
}

docker compose ps -a
Write-Host "Hotfix verification passed." -ForegroundColor Green
Write-Host "Re-run the failed proposal evaluation; uploaded package documents remain valid."
